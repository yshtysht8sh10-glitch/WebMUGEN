...
describe('FreshStageRenderer', () => {
  ...
  it('moves the distant view slowly and locks the arena floor to the full camera delta', () => {
    vi.stubGlobal('Image', PanoramaImage);
    const initialDraw = vi.fn();
    const movedDraw = vi.fn();
    const renderer = new FreshStageRenderer();
    ...
    expect(firstShift).toBeCloseTo(-6);
    expect(lastShift).toBeCloseTo(-50);
    const shifts = movedDraw.mock.calls.map((call, index) => (call[1] as number) - (initialDraw.mock.calls[index][1] as number));
    for (let index = 1; index < shifts.length; index += 1) {
      expect(shifts[index]).toBeLessThanOrEqual(shifts[index - 1] + 0.001);
    }
  });
});
...
function stage() {
  return parseWebMugenStage({
    format: 'webmugen-stage', version: 1, id: 'fresh', name: 'Fresh', presentation: 'fresh', groundY: 0,
