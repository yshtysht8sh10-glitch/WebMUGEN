...
export function processSoundRuntimeEvents(
  ...
  for (const event of events) {
    if (diagnosticsEnabled) lines.push(`raw.sound_runtime_event type=${event.type} owner=${event.ownerId} generated=1`);
    if (event.type === 'stop') {
      if (event.channel === null) {
        if (diagnosticsEnabled) lines.push(`raw.sound_stop owner=${event.ownerId} channel=- result=noop reason=channel_missing`);
        continue;
      }
      const stopped = runtime?.stopChannel(`${event.ownerId}:${Math.trunc(event.channel)}`) ?? false;
      if (diagnosticsEnabled) lines.push(`raw.sound_stop owner=${event.ownerId} channel=${Math.trunc(event.channel)} result=${stopped ? 'stopped' : 'noop'}${stopped ? '' : ' reason=channel_not_found'}`);
      continue;
    }
    if (event.type === 'pan') {
      if (event.channel === null) {
        if (diagnosticsEnabled) lines.push(`raw.sound_pan owner=${event.ownerId} channel=- result=noop reason=channel_missing`);
        continue;
      }
      if (event.pan === null || event.mode === null) {
        if (diagnosticsEnabled) lines.push(`raw.sound_pan owner=${event.ownerId} channel=${Math.trunc(event.channel)} result=noop reason=pan_missing`);
        continue;
      }
      ...
      if (diagnosticsEnabled) lines.push(`raw.sound_pan owner=${event.ownerId} channel=${Math.trunc(event.channel)} mode=${event.mode} raw=${event.pan} normalized=${normalized} result=${result === 'updated' ? 'updated' : 'noop'}${result === 'updated' ? '' : ` reason=${result}`}`);
      continue;
    }
    ...
    if (diagnosticsEnabled) lines.push(`raw.sound_lookup owner=${event.ownerId} scope=${event.scope} sample=${scopePrefix}${event.group},${event.index} result=found bytes=${sample.bytes.byteLength}`);
    ...
    if (!runtime || (runtime.status !== 'unlocked' && !runtime.isUnlockPending)) {
      if (diagnosticsEnabled) lines.push(`raw.sound_play_rejected owner=${event.ownerId} sample=${scopePrefix}${event.group},${event.index} reason=audio_locked${runtimeDetails}`);
      continue;
    }
    ...
    if (diagnosticsEnabled) lines.push(`raw.sound_play owner=${event.ownerId} scope=${event.scope} sample=${scopePrefix}${event.group},${event.index} channel=${event.channel ?? '-'} volume=${event.volume} volumescale=${event.volumeScale} pan=${event.pan} freqmul=${event.frequencyMultiplier} loop=${event.loop ? 1 : 0} result=queued${runtimeDetails}`);
  }
  return lines;
}
