...
describe('CharacterLoader common movement routes', () => {
  ...
  it('does not let common walk interrupt a special standing state with transient control', async () => {
    const character = await loadCharacterFromDef('/chars/kfm/kfm.def', createCommonRouteTestFetcher());
    const state = createInitialGameState();
    const result = stepCnsStateRuntime(
      {
        ...state,
        players: [
          { ...state.players[0], stateNo: 60011, stateType: 'S', physics: 'S', ctrl: true },
          state.players[1],
        ],
      },
      character.cns,
