...
- `AnimElem`: uses the AIR action's 1-based element number and is true on the frame where that element starts. A finite action with no `LoopStart` repeats the whole action and re-enters bare `AnimElem = N` on each pass. Explicit `LoopStart` and a negative-duration element retain the original non-resetting trigger timeline; an infinite terminal element therefore fires only when first reached. `AnimElemTime` and the legacy `AnimElem = N, op T` form continue to compare the original AIR-relative time and reject out-of-range element numbers. Bundled T-H-M-A Helper State 3031 repeatedly creates Action 3030, while State 3940's infinite element 11 enters TargetState 3935 only once.
