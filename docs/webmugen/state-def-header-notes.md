...
| `facep2` | Complete | Parsed and applied on state entry; zero preserves incoming Facing and one faces the opponent. Later grounded cross-overs and landing are handled independently by Stage AutoTurn, while airborne Facing is preserved; `noautoturn` and pause freezes are respected. Debug Overlay exposes facing. | Helper-specific AutoTurn behavior still needs verification. |
