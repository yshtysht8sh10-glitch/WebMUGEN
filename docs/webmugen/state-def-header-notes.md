...
| `facep2` | Complete | Parsed and applied on state entry; zero preserves incoming Facing and one faces the opponent. Later cross-overs are handled independently by Stage AutoTurn, with `noautoturn` and pause freezes respected. Debug Overlay exposes facing. | Helper-specific AutoTurn behavior still needs verification. |
