...
| Player collision | `PlayerPush`, `Width`, `ScreenBound` | `PlayerPush = 0` disables separation for its execution frame. Grounded players retain horizontal push; airborne players may cross after their Size-derived Push Boxes clear vertically. Root `ScreenBound` feeds the shared 320/400x240 camera: `value = 1` keeps the Push Box inside the horizontal viewport, `value = 0` permits off-screen movement, and `movecamera` selects per-axis camera influence without clearing velocity. External stages now apply DEF camera bounds/tension/vertical follow/floor tension and Bound screen insets. `Width`, Helper/custom-state ownership, exact camera hysteresis, and PlayerInfo bounds remain incomplete. |
