...
| Helper/Projectile/Explod | `Helper`, `Projectile`, `Explod`, `ModifyExplod`, `RemoveExplod`, `ExplodBindTime` | Explod creation now evaluates through production CNS into owner-scoped `GameState` entries with separate runtime/MUGEN ids, round cleanup, and diagnostics. Rendering/lifecycle and the three mutation controllers remain Partial/safe no-op; see `explod-integration-design.md`. |
