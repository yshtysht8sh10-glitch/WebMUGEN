...
| State transition | `ChangeState`, `SelfState` | `ChangeState` preserves current State ownership and applies its `ctrl` value before destination StateDef entry, so an explicit destination `ctrl` wins while omission inherits the controller value. `SelfState` returns borrowed players to their own CNS document. Helper and animation ownership remain Partial. |
...
| Velocity/position | `VelSet`, `VelAdd`, `VelMul`, `PosSet`, `PosAdd` | `VelSet`/`VelAdd` X values are converted from facing-relative CNS coordinates to world velocity when applied. `VelMul` scales that stored world velocity without applying facing again. `PosSet` evaluates absolute X/MUGEN Y expressions with player/opponent redirect context; `PosAdd` evaluates the same expression surface and converts X from Facing-relative to world position. Bundled T-H-M-A State 3110 covers turn, `Enemy(0), Pos X/Y`, and the final behind-opponent offset. |
| State flags | `CtrlSet`, `StateTypeSet`, `MoveTypeSet`, `AssertSpecial` | `CtrlSet` persists while the current State remains active, including from a positive State into the following tick's State -1 scan; active hit stun still suppresses premature control. All three AssertSpecial slots are retained per entity for the asserted tick and deassert before the next tick. Facing/invisible, stand/crouch/air guard prohibition, unguardable, juggle bypass, stage background, and HUD consumers are connected. Round/timer/audio/shadow/noFG/nowalk consumers and exact hitpause persistence remain incomplete. |
...
| Player collision | `PlayerPush`, `Width`, `ScreenBound` | `PlayerPush = 0` disables separation for its execution frame. `Width` evaluates tick-scoped front/back `edge` and `player` pairs (plus the shared `value` shorthand): edge widths feed screen containment/body-edge distance and player widths feed push separation. Grounded players retain horizontal push; airborne players may cross after their resolved Push Boxes clear vertically. Root `ScreenBound` feeds the shared 320/400x240 camera: `value = 1` keeps the edge bar inside the horizontal viewport, `value = 0` permits off-screen movement, and `movecamera` selects per-axis camera influence without clearing velocity. External stages apply DEF camera bounds/tension/vertical follow/floor tension and Bound screen insets. Negative Width values, Helper/custom-state ownership, exact camera hysteresis, and PlayerInfo bounds remain incomplete. |
...
| Visual/audio effects | `AfterImage`, `BGPalFX`, `AllPalFX`, `PalFX`, `EnvColor`, `EnvShake`, `GameMakeAnim`, `MakeDust`, `PlaySnd`, `StopSnd`, `SndPan`, `Pause`, `SuperPause`, `Trans`, `AngleDraw` | `AfterImage` captures position/facing/Anim history and renders gap-selected frames. `BGPalFX` scopes its timed filter to the background, including external SFF stage layers; it does not hide the stage image. `AssertSpecial noBG` remains a separate, intentional background-suppression command. `PalFX` applies the shared timed Canvas filter to a player; `AllPalFX` applies it to both root players and the stage. The shared Canvas approximation applies `invertall` before multiply/add, so bundled T-H-M-A State 3169's `invertall = -1` plus `mul = 0,0,0` ends black as in WinMUGEN. `AngleDraw` applies tick-scoped character angle/scale around the player axis. `EnvColor` renders a timed under/over RGB layer. `GameMakeAnim` and `MakeDust` feed legacy fightfx actions into the shared Explod path. `EnvShake` evaluates WinMUGEN parameters/defaults and starts shared Canvas shake. Character audio, HitDef sounds, and match-level Pause/SuperPause are production-connected. During Pause/SuperPause, only entities allowed by root `movetime` or Helper `pausemovetime`/`supermovetime` may resolve HitDef collision; bundled T-H-M-A State 3100→3110 verifies contact before SuperPause ends. Exact indexed-palette arithmetic, BGPalFX/AllPalFX activation during hitpause, bundled fightfx/common sound archives, same-pass pause ordering, Helper ownership, and exact pan remain Partial. `Zoom` is version-gated as MUGEN 1.1 and not applicable to the WinMUGEN target. |
