...
| Hit-related | `HitDef`, `HitBy`, `NotHitBy`, `HitVelSet`, `HitFallVel`, `HitFallDamage` | `HitDef` evaluates a typed activation snapshot for major fields. Damage, ground/air hit time, animation selection, velocity, fall/recover data, and basic down time reach the live common-state path. `HitVelSet`/`HitFallVel` restore snapshotted velocity components. Guard and advanced fall/down behavior remain Partial. |
