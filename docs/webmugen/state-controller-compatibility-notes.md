...
| Helper/Projectile/Explod | `Helper`, `Projectile`, `Explod`, `ModifyExplod`, `RemoveExplod`, `ExplodBindTime` | Explod create/render/lifecycle, explicit-ID mutations, motion/render extensions, owner-hit removal, Pause gating, and isolated HitDef spark entries are production-connected. Hit sparks share lifecycle/Canvas but remain outside controller ID selection. Common fightfx assets, ownpal isolation, destination/subtractive blend, shadow pass, and non-player owners remain Partial. |
| Visual/audio effects | `AfterImage`, `PalFX`, `EnvShake`, `PlaySnd`, `StopSnd`, `SndPan`, `Pause`, `SuperPause`, `Trans`, `AngleDraw` | Character audio, HitDef `S`-scope hit/guard sounds, and match-level Pause/SuperPause are production-connected through shared runtime bridges. Issue #51 preserves PlaySnd/HitDef events emitted while the first user-gesture AudioContext resume is pending without creating a pre-gesture queue. Common sound archives, same-pass pause ordering, Helper ownership, full SuperPause visuals, and exact pan remain Partial. Other visual effects are mostly safe no-op or field storage. |
