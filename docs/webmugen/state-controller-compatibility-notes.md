...
| Helper/Projectile/Explod | `Helper`, `Projectile`, `Explod`, `ModifyExplod`, `RemoveExplod`, `ExplodBindTime` | Explod create/render/lifecycle and explicit-ID mutations are production-connected. Unbound velocity/acceleration, injected creation random, scale, additive source alpha, and removeongethit are active. Ownpal isolation, destination/subtractive blend, shadow pass, non-player owners, and pause integration remain Partial; see `explod-integration-design.md`. |
