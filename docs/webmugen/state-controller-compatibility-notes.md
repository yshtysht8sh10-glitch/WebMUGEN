...
| Visual/audio effects | `AfterImage`, `PalFX`, `EnvShake`, `PlaySnd`, `Trans`, `AngleDraw` | PlaySnd character SND playback is production-connected with owner channels and major parameters; common sound/Helper ownership and StopSnd/SndPan remain Partial. Other visual effects are mostly safe no-op or field storage. |
