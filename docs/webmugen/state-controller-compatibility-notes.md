...
| Visual/audio effects | `AfterImage`, `PalFX`, `EnvShake`, `PlaySnd`, `StopSnd`, `SndPan`, `Pause`, `SuperPause`, `Trans`, `AngleDraw` | `AfterImage` captures position/facing/Anim history and renders gap-selected frames. Character audio, HitDef sounds, and match-level Pause/SuperPause are production-connected. Exact palette arithmetic, common sound archives, same-pass pause ordering, Helper ownership, and exact pan remain Partial. |
