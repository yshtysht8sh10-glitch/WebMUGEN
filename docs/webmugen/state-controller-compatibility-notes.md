...
| Helper/Projectile/Explod | `Helper`, `Projectile`, `Explod`, `ModifyExplod`, `RemoveExplod`, `ExplodBindTime` | Explod creation, owner AIR/SFF rendering, lifecycle, round cleanup, explicit-ID partial modification, and explicit-ID same-frame removal are production-connected with ordered diagnostics. Omitted Modify/Remove id selection, movement/pause integration, and ExplodBindTime remain Partial; see `explod-integration-design.md`. |
