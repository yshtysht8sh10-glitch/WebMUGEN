...
| Visual/audio effects | `AfterImage`, `PalFX`, `EnvShake`, `PlaySnd`, `StopSnd`, `Trans`, `AngleDraw` | PlaySnd character playback and StopSnd owner/channel stopping are production-connected. Common sound/Helper ownership, omitted StopSnd channel, and SndPan remain Partial. Other visual effects are mostly safe no-op or field storage. |
