...
| Helper/Projectile/Explod | `Helper`, `Projectile`, `Explod`, `ModifyExplod`, `RemoveExplod`, `ExplodBindTime` | Explod creation, owner AIR/SFF rendering, age/AnimElem/time, removetime, bind release, and round cleanup are production-connected with diagnostics. Movement/pause extensions and the three mutation controllers remain Partial/safe no-op; see `explod-integration-design.md`. |
