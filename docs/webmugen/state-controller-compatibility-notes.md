...
| Helper/Projectile/Explod | `Helper`, `Projectile`, `Explod`, `ModifyExplod`, `RemoveExplod`, `ExplodBindTime` | Explod create/render/lifecycle, explicit-ID mutations, motion/render extensions, owner-hit removal, and Pause/SuperPause per-entry allowance gating are production-connected. Ownpal isolation, destination/subtractive blend, shadow pass, and non-player owners remain Partial; see `explod-integration-design.md`. |
| Visual/audio effects | `AfterImage`, `PalFX`, `EnvShake`, `PlaySnd`, `StopSnd`, `SndPan`, `Pause`, `SuperPause`, `Trans`, `AngleDraw` | Character audio and match-level Pause/SuperPause are production-connected. Pause gates negative/current CNS, physics, round/hit stepping, and Explods; already-started audio continues without time-zero replay. Same-pass ordering, common sound/Helper ownership, full SuperPause visuals, and exact pan remain Partial. Other visual effects are mostly safe no-op or field storage. |
