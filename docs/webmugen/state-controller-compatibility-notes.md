...
| Helper/Projectile/Explod | `Helper`, `Projectile`, `Explod`, `ModifyExplod`, `RemoveExplod`, `ExplodBindTime` | Explod creation evaluates through production CNS and owner-scoped entries render owner AIR/SFF frames with position/Facing/order diagnostics. Lifecycle and the three mutation controllers remain Partial/safe no-op; see `explod-integration-design.md`. |
