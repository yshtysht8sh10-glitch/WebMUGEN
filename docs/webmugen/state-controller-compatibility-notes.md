...
| Helper/Projectile/Explod | `Helper`, `Projectile`, `Explod`, `ModifyExplod`, `RemoveExplod`, `ExplodBindTime` | Explod create/render/lifecycle and explicit-ID modify/remove/bind-time operations are production-connected with ordered diagnostics. Bind time zero/positive/negative and rebind are covered. Omitted-id selection, non-player owner lifecycle, and movement/pause integration remain Partial; see `explod-integration-design.md`. |
