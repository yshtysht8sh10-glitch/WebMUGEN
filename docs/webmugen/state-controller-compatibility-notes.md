...
| Combat interception | `AttackDist`, `HitOverride`, `ReversalDef`, `HitFallSet` | AttackDist updates the live HitDef guard range. HitOverride uses timed attribute slots and replaces normal damage with a configured self State. ReversalDef uses Clsn1-vs-Clsn1, incoming attribute filtering, custom states, pause, Target registration, and MoveReversed without aliasing normal HitDef. HitFallSet remains connected to the common fall snapshot. Helper/projectile parity, reversal effects, and exact pause/priority ordering remain Partial. |
