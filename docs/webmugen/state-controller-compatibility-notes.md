...
| State flags | `CtrlSet`, `StateTypeSet`, `MoveTypeSet`, `AssertSpecial` | Basic state flags exist. All three AssertSpecial slots are retained per player for the asserted tick and deassert before the next tick; `noautoturn` suppresses grounded auto-facing. Consumers for render/guard/round/audio flags remain incomplete. |
