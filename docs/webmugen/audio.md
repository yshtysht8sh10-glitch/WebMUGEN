...
Issue #51 closes both sides of the first-gesture race. Pointer input still reaches the app-level `pointerdown` unlock listener. Keyboard input no longer depends on a separate window listener: `BrowserInput` invokes an `onUserGesture('keydown')` callback in the same handler that records a non-editable game key. The callback is runtime-agnostic; `WebMugenApp` connects it to the shared audio unlock handler. Input, select, textarea, and content-editable keydowns neither enter game input nor request audio unlock. Concurrent/repeated callbacks share one in-flight `resume()` and one AudioContext. A PlaySnd or HitDef sound emitted while that gesture's resume is pending waits for the same bounded Promise and starts after a successful unlock; sound emitted before any gesture is still rejected instead of being accumulated. A resolved `resume()` is successful only when the adapter subsequently reports `running`. A resolved-but-still-`suspended` context remains locked, clears the pending attempt, and can retry on the next gesture. Cleanup invalidates the callback and in-flight attempt, so an old adapter cannot unlock or play after unmount. Runtime tab changes are not part of this lifecycle.
