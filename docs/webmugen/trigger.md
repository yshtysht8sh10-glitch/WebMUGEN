...
`Ctrl` remains supported. `HitShakeOver = 1` likewise exposes the defender hit-pause completion
flag numerically while preserving the bare `HitShakeOver` form. Jecht's custom target State 3405
depends on this comparison to enter State 3406 and launch the opponent after hit shake.
...
- `NumHelper` counts Helpers owned by the current root and optionally filters by MUGEN Helper ID; every accepted Helper is immediately count-visible and executes its initial current-State pass on the creation frame, including Helpers generated by another Helper;
...
MoveContact, MoveHit, MoveGuarded, and HitCount read attacker-side move results. A normal hit sets MoveContact/MoveHit to 1 and increments the State-local count; a live guardflag-approved contact sets MoveContact/MoveGuarded to 1 without MoveHit or hit-count increment. The Move* value remains 1 throughout attacker hitpause, then increments on each unpaused game tick. MoveContact, MoveHit, MoveGuarded, and MoveReversed return zero outside an attack State (`MoveType != A`), even when a delayed Projectile has left a stored contact result on the player. Activating a later HitDef does not clear the preceding result; its next hit or guard replaces the result and resets the value to 1. On State entry, `movehitpersist` controls result preservation and `hitcountpersist` controls count preservation independently. `MoveHitReset` clears flags without erasing target hit history or count.

Projectile contact uses the same attacker-side MoveContact result shape. This lets bundled T-H-M-A State 1005 execute its `MoveGuarded` VarSet branches and its `MoveHit`-gated PowerAdd after projectile contact while State 1005 remains an attack State. If the Projectile lands later while its owner is idle, the stored contact remains available to Projectile-specific history but Move* evaluates to zero. This prevents T-H-M-A's damage-counter Helper from repeatedly routing 5506 -> 5504 -> 5506 and lets its Time=200 DestroySelf execute.
...
Issues #118/#127 verify that a redirect changes the command source as well as the PlayerState source. In a non-keyctrl Helper, bare `Command` has no root input, while `root, Command = "..."` reads the root's resolved command set. A `keyctrl = 1` Helper receives its owning root's resolved commands both in its own State -1 and when another entity evaluates `helper(ID), Command = "..."`; `keyctrl = 0` Helpers remain command-inert. D4meirin depends on the latter form for its root attack routes. This is general redirect execution behavior and contains no character or state-number branch.
