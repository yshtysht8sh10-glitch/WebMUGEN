...
- **Local:** uses `showDirectoryPicker()` when supported. Independently choose external Character, Stage, and LifeBar folders; any source may remain unset. The Generator recursively reads candidates and maps their relative paths to the deployed application's `chars`, `stages`, and `lifebars` directories. At the origin root these remain `/chars`, `/stages`, and `/lifebars`; a subdirectory deployment includes its application prefix. Previously stored root-only defaults migrate to that current prefix. An optional output folder enables direct `catalog.json` save; download remains available as a fallback.
