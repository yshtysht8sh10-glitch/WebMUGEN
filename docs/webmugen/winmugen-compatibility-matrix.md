...
| ctrl | Control flag | Complete | Parsed and applied. An explicit `ChangeState ctrl` or `CtrlSet` may override the StateDef value. Issue #56 scopes WebMUGEN's injected common walk-start route to Stand State 0, so transient control in a special standing State cannot incorrectly route it to State 20/21. |
