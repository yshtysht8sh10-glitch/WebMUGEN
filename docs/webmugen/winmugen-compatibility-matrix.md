...
| 5000 | Common get-hit state | Partial 80% | Implemented: Standing hit enters State 5000 with persistent GetHitVar snapshot, required Anim selection, independent hit-stun clock, and guarded internal transitions. Air/crouch/fall/guard branches remain incomplete. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
| 5001 | Common get-hit state | Not started | Required for hit reactions. |
| 5010 | Common get-hit state | Not started | Required for hit reactions. |
| 5011 | Common get-hit state | Not started | Required for hit reactions. |
| 5020 | Common get-hit state | Partial 40% | Implemented: Air contact enters the unmodified common state with air animtype/hittime/velocity snapshot data. Guard and uncommon animation branches remain incomplete. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
| 5030 | Fall state | Partial 75% | Implemented: Common air hit velocity/gravity and HitFall routing are covered by runtime integration tests. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
| 5035 | Fall state | Partial 60% | Implemented: Common transition participates in tested air-hit recovery/fall paths; animation completeness varies by character. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
| 5040 | Fall state | Partial 55% | Implemented: Non-fall air recovery and landing are connected; full KO and animation behavior remains incomplete. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
| 5050 | Fall state | Partial 55% | Implemented: Fall/recover/recovertime routes and landing detection are connected; recovery state details remain Partial. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
| 5060 | Fall state | Not started | Required for knockdown. |
| 5070 | Fall state | Not started | Required for knockdown. |
| 5080 | Fall recovery state | Not started | Required for recovery. |
| 5090 | Fall recovery state | Not started | Required for recovery. |
| 5100 | Down state | Partial 40% | Implemented: Falling contact can enter the common bounce/down path without ground clamping hiding the crossing. Effects and full bounce variants remain incomplete. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
| 5110 | Down state | Partial 40% | Implemented: Common liedown path is reachable and `down.hittime` schedules State 5120. Full down velocity and animation variants remain incomplete. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
| 5120 | Recovery state | Not started | Required for recovery. |
| 5150 | Dead state | Not started | Required for KO behavior. |
| 5200 | Lie dead state | Not started | Required for KO behavior. |
| 5210 | Lie dead state | Not started | Required for KO behavior. |
...
| Explod | Partial 85% | Implemented: Issue #37 verifies production CNS/GameState/AIR renderer paths with KFM, T-H-M-A, and Yes030_e-rada for P1/P2, both Facings, and 1,800 lifecycle ticks. Common fightfx assets, palette/blend/shadow edges, camera exactness, Helper ownership, generic `persistent`, and `NumExplod` remain incomplete. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
...
| FallEnvShake | Safe no-op | Recognized by the runtime without changing game state. Recognized safe no-op. Landing shake parameters are not connected to HitFeedback. |
...
| HitFallDamage | Partial 55% | Implemented: Applies snapshotted fall.damage at the common-State trigger and honors fall.kill; fall envshake/projectile parity remains incomplete. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
| HitFallSet | Safe no-op | Recognized by the runtime without changing game state. Recognized safe no-op. Fall flags not implemented. |
...
| NotHitBy | Partial 40% | Implemented: Matching normalized HitDef state/attack attr rejects live contact. Duration/stacking remains incomplete. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
...
| PlayerPush | Fallback 45% | Implemented: `value = 0` disables fallback stage push for its execution frame. Grounded players always use horizontal push; airborne cross-over requires fixed 44x80 boxes to clear vertically. Width/AIR `Clsn2` integration remains incomplete. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current approximate behavior, runtime inventory, and focused-test inventory. |
| PlaySnd | Partial 85% | Implemented: Issue #37 resolves real SND samples for KFM, T-H-M-A, and Yes030_e-rada through production CNS for P1/P2. Pause boundaries and Browser Audio are connected. Common `F` archive, lowpriority, Helper ownership, and same-pass cross-player ordering remain Partial. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
| PosAdd | Complete | Basic implementation exists. |
| PosFreeze | Safe no-op | Recognized by the runtime without changing game state. Recognized safe no-op. Freeze behavior not implemented. |
...
| TargetPowerAdd | Partial 55% | Implemented: Adds and lower-clamps power on registered targets selected by optional HitDef id. Full power-limit and Helper/multi-player semantics remain incomplete. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
...
| down.hittime | Partial 40% | Implemented: Schedules common liedown/getup timing; advanced down branches remain incomplete. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
| down.velocity | Partial 40% | Implemented: Snapshotted for common down/fall paths; bounce variants remain incomplete. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
...
| fall | Partial 40% | Implemented: Drives common air-fall routing and GetHitVar; advanced variants remain incomplete. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
...
| fall.recover | Partial 40% | Implemented: Controls common air recovery eligibility. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
| fall.recovertime | Partial 40% | Implemented: Controls common air recovery timing. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
...
| guardsound | Partial 40% | Implemented: Guard contact creates one shared SoundPlayEvent. `S` scope resolves attacker SND through Browser Audio; common/`F` scope requires a common SND archive. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
...
| getpower | Partial 40% | Implemented: Explicit hit/guard values are applied once to the attacker and clamped to 0..3000. mugen.cfg-derived omitted defaults remain unavailable. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
| givepower | Partial 40% | Implemented: Explicit hit/guard values are applied once to the defender and clamped to 0..3000. mugen.cfg-derived omitted defaults remain unavailable. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
...
| sparkno | Partial 55% | Implemented: Normal hit creates one shared Explod effect at the contact point. `S` scope uses attacker AIR/SFF; common/`F` scope requires fightfx assets and is diagnosed when absent. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
| sparkxy | Partial 55% | Implemented: Applies Facing-relative X and direct Y offset to the Clsn contact point before same-frame Explod rendering. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
...
| CanRecover | Partial 55% | Implemented: Reads fall.recover and fall.recovertime during the common air-fall path. Broader recovery command/state behavior remains incomplete. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
...
| GetHitVar | Partial 80% | Implemented: Contact snapshot supplies damage, hit/slide/control time, velocity, type/anim codes, fall damage/kill, combo hitcount, snap xoff/yoff, ids, guarded, and yaccel across get-hit State changes. Unsupported zoff/fall-time keys are diagnosed safe defaults. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
...
| Lose | Safe no-op | Recognized by the runtime without changing game state. Safe default false. Round result not implemented. |
...
| MatchOver | Not started | Match flow not implemented. |
...
| PowerMax | Partial 40% | Implemented: Default 3000. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
...
| Win | Safe no-op | Recognized by the runtime without changing game state. Safe default false. Round result not implemented. |
