...
| hitflag | Partial 75% | Implemented: H/L/M/A/F/D target classes are connected, and the legacy `P` suffix is tolerated without changing the target-class match. Missing: `+`/`-` modifiers and remaining WinMUGEN semantics. Evidence: focused target-class tests plus bundled T-H-M-A State 410/610 `MAFP` collision coverage. |
