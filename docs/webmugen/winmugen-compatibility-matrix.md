...
| Explod | Partial 90% | Implemented: Issues #37/#45 verify production CNS/GameState/AIR paths plus SFF v1 shared and sprite-specific palettes, transparent index, normal/Explod owner RGBA parity, cross-owner cache isolation, and separate `ownpal` surfaces. Real KFM State 191 pixels are non-black. SFF v2, dynamic palette effects, destination/subtractive blend, shadows, common fightfx assets, camera exactness, Helper ownership, generic `persistent`, and `NumExplod` remain incomplete. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
