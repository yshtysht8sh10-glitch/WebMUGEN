...
| AssertSpecial | Partial 40% | Implemented: `flag`/`flag2`/`flag3` recognize `noautoturn`, retain Facing through the asserted game tick including a same-tick ChangeState, and automatically deassert before the next CNS tick. Ground stage auto-facing honors the flag and `raw.cross` exposes it. Missing: all other AssertSpecial flags, hitpause/version-specific persistence, and broader Helper behavior. Evidence: focused state-transition/stage-facing test. |
...
| Explod | Partial 94% | Implemented: Issues #37/#45/#46 verify production CNS/GameState/AIR paths plus SFF v1 shared and sprite-specific palettes, subfile-order `samePalette` inheritance, linked sprite palette context, ACT-only reversed index lookup, transparent index, AIR Preview/normal/Explod owner RGBA parity, cross-owner/cache-key isolation, and separate `ownpal` surfaces. Issue #81 preserves the AIR element seventh-field blend and uses it when Explod `trans` is omitted or `default`; real T-H-M-A Action 3301 is regression-tested through extracted AIR and the production ZIP loader. Bare AIR `A` uses Canvas `lighter` with a diagnosed 50% source-alpha approximation so ghost Explods remain visibly transparent. Real KFM State 191 and T-H-M-A Action 15001 sprite pixels are non-black with expected palette metadata. Exact bare-`A` coefficients, SFF v2, dynamic palette effects, destination/subtractive blend, shadows, common fightfx assets, camera exactness, Helper ownership, generic `persistent`, and `NumExplod` remain incomplete. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
...
| Helper | Partial 65% | Implemented: Issue #58 connects CNS Helper controllers to independent entities with unique runtime IDs, ownership, deferred State execution, nested spawning, rendering, NumHelper, DestroySelf, diagnostics, special-State ordering, and round cleanup. Issue #81 resolves active Helper HitDefs against the opposing root's AIR body boxes and returns damage/reaction/effects plus Helper-local HitPause, MoveHit, consumed-target, and Target registration; bundled T-H-M-A State/Action 3320 is regression-tested. Missing: Helper-as-defender and Helper-vs-Helper combat, push collision, exact Helper/root Power ownership, redirects, Bind/ParentVar, palette mutation, and pause semantics. Evidence: focused synthetic and real-character Helper HitDef integration tests plus existing lifecycle/runtime coverage. |
...
| HitDef | Partial 85% | Implemented: Issue #36 routes one normal/guard contact effect per HitEvent into shared Explod and Sound runtimes. `S`-scoped expression animations/sounds use attacker AIR/SFF/SND; unprefixed/`F` use injected common assets. CNS HitDef activation and Clsn lookup now share the pre-physics animation snapshot while retaining post-physics positions, with bundled T-H-M-A State 215 one-tick `AnimElem = 4` coverage. Bundled common fightfx/SND assets, Helper/projectile parity, modifiers, and mixed priority edges remain incomplete. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: current implemented behavior, runtime inventory, and focused-test inventory. |
...
| Projectile | Partial 60% | Implemented: the production CNS runtime emits Projectile entries; the app advances Facing-relative `velocity` and `accel`, honors `projanim`, `projscale`, `projremovetime`, renders owner AIR, derives the initial AIR Clsn1 hit box, and applies a basic damaging contact/removal path. Missing: animated collision-box changes, guard/full HitDef parity, `projremove` variants, hit/guard/cancel animations, projectile priority/interactions, pause semantics, and remaining parameters. Evidence: focused controller/motion/collision tests and bundled T-H-M-A State 1000/1005 data. |
