...
| PlaySnd | Partial 85% | Implemented: Issue #37 resolves real SND samples for KFM, T-H-M-A, and Yes030_e-rada through production CNS for P1/P2. Pause boundaries and Browser Audio are connected. Issue #44 adds persistent master/mute controls. Issue #51 shares one first-gesture resume and retains the following PlaySnd request until that attempt resolves, without a pre-gesture queue or Runtime-tab dependency. These app lifecycle fixes do not raise controller compatibility progress. Common `F` archive, lowpriority, Helper ownership, and same-pass cross-player ordering remain Partial. Missing: remaining WinMUGEN semantics not identified as covered by this row. Evidence: focused BrowserAudioRuntime, app gesture lifecycle, and SoundRuntimeBridge tests plus production runtime inventory. |
