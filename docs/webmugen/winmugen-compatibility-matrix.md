...
| 5000 | Common get-hit state | Complete | Issue #59 verifies standing contact entry, High/Low and all six animtypes, optional Up/DiagUp fallback, velset/header order, persistent GetHitVar, real HitShakeOver, launch/fall branches, P1/P2, Facing, hitpause, and terminal Controller order against the unmodified common1.cns. |
| 5001 | Common get-hit state | Complete | Issue #59 verifies ground slide entry, HitVelSet X restore, AnimTime continuation, slidetime friction, independent HitOver, early-ctrl suppression, P1/P2 symmetry, and the common State 0 ctrl=1 exit. |
| 5010 | Common get-hit state | Complete | Issue #59 verifies crouching contact entry, six animtype branches, StateTypeSet launch/fall behavior, persistent snapshot, real HitShakeOver, and terminal routing to 5011 or 5030 against the unmodified common1.cns. |
| 5011 | Common get-hit state | Complete | Issue #59 verifies crouch slide HitVelSet/Anim/slidetime/HitOver behavior, early-ctrl suppression, and the common State 11 ctrl=1 exit. |
| 5020 | Common get-hit state | Complete | Issue #59 verifies air-only contact entry, air.type and air.animtype selection including Back/Up/DiagUp fallback, air hittime/velocity snapshot, hitpause clock separation, real HitShakeOver, and terminal routing to 5030. Guard contact remains correctly isolated in States 150/152/154. |
| 5030 | Fall state | Complete | Issue #60 verifies HitVelSet, per-frame GetHitVar(yaccel), HitOver/HitFall routing, animation completion, unclamped downward ground crossing, hitpause, and persistent HitDef snapshot against unmodified common1.cns. |
| 5035 | Fall state | Complete | Issue #60 verifies optional transition animation selection, gravity, non-fall/fall routing, no-transition behavior, ground crossing, and terminal controller order. |
| 5040 | Fall state | Complete | Issue #60 verifies non-fall air recovery, KO fallback, Anim/CtrlSet/StateTypeSet behavior after HitOver, gravity, and the State 52 landing route. |
| 5050 | Fall state | Complete | Issue #60 verifies 5050-to-5060 descending animation selection, gravity, independent fall.recover/recovertime plus recovery input gating, and ground-crossing routing to bounce/down. Recovery destination States 5200/5210 are tracked separately. |
| 5060 | Fall animation family | Complete | WinMUGEN common1.cns has no StateDef 5060. Issue #60 verifies that 5060 is selected as the descending animation while State 5050 remains active; no fabricated StateDef or state-number hack is used. |
| 5070 | Trip fall state | Complete | Issue #60 verifies State 5070 hit-shake freeze and terminal transition to 5071, then stored HitVelSet, yaccel, and ground routing in 5071. |
| 5080 | Downed get-hit state | Complete | Issue #61 verifies lying-hit entry, HitShakeOver freeze, yvel-based 5080/5090 animation choice, down.hittime/down.velocity, and terminal routing to 5081 or 5030 for P1/P2. |
| 5081 | Downed slide state | Complete | Issue #61 verifies HitVelSet X, HitOver stop, ground friction, sysvar ground-frame suppression, and terminal State 5110 routing. |
| 5090 | Downed launch animation family | Complete | WinMUGEN common1.cns has no StateDef 5090. Issue #61 verifies that State 5080 selects Anim 5090 for a lying target launched by nonzero down.velocity.y, with fallback to Anim 5030 only when 5090 is absent. |
| 5100 | Down state | Complete | Issue #61 verifies unclamped falling contact, landing animation variants, ground positioning, velocity reduction, fall damage, down.bounce/no-bounce selection, and terminal routing to 5101 or 5110. |
| 5101 | Bounce state | Complete | Issue #61 verifies HitFallVel restoration, default fall.yvelocity, bounce positioning, per-frame acceleration, and the second ground crossing into State 5110. |
| 5110 | Down state | Complete | Issue #61 verifies landing animation variants, fall damage, ground position/velocity, friction, KO routing ownership, and an independent Data.liedown.time clock. down.hittime is correctly used only as lying-hit slide time. |
| 5120 | Recovery state | Complete | Issue #61 verifies get-up animation variants, zero X velocity, timed two-slot NotHitBy immunity, HitFallSet, terminal State 0 ctrl=1, and suppression of automatic get-up when Life is zero. |
...
| HitBy | Complete | Issue #61 implements both WinMUGEN attribute slots, value/value2 selection, independent timers, pause freeze, and all-active-slot matching. |
...
| HitFallSet | Complete | Issue #61 applies value/xvel/yvel to the persistent fall snapshot and verifies the common State 5120 use. |
...
| NotHitBy | Complete | Issue #61 implements both WinMUGEN attribute slots, state-only and attack-only filters, independent timers, pause freeze, and the State 5120 get-up immunity sequence. |
...
| down.hittime | Complete | Issue #61 selects lying-hit hit-stun/slide time only when StateType=L; it no longer incorrectly schedules get-up. |
| down.velocity | Complete | Issue #61 selects lying-hit X/Y velocity, defaults it to air.velocity, and routes zero-Y slide versus nonzero-Y launch through State 5080. |
| down.bounce | Complete | Issue #61 selects no-bounce fall.yvel=0 or one-bounce fall velocity for a launched lying target. |
