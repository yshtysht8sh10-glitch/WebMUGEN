...
| Helper | Partial 50% | Implemented: Issue #58 connects CNS Helper controllers to an independent GameState entity collection with unique runtime IDs separate from duplicate MUGEN IDs, root/parent/owner/state/animation ownership, requested State/Anim, deferred first step, P1/P2 and nested spawning, owner AIR/SFF rendering, NumHelper, DestroySelf, diagnostics, and round-reset cleanup. The special-State Phase 1 keeps State -3/-2 root-only, skips State -1 for `keyctrl = 0`, and applies root command State -1 before the Helper current State for `keyctrl = 1`. Phase 2 gates `debugControllerCheck` before invocation when trace diagnostics are disabled, reducing a three-Trigger production route from 21 evaluations to 3 while preserving all 32 debug-enabled evaluations and output. Phase 3 caches `triggerall` and numbered AND/OR groups at parse completion, preserving first-seen evaluation order and eliminating runtime grouping arrays. Phase 4 compiles Trigger boolean/comparison/arithmetic/Redirect structure after the final CNS merge; compiled/legacy results match all 6,733 local itoko Trigger records. Phase 5 indexes StateNo with first-duplicate precedence and uses Map lookup throughout core Runtime/Helper/Physics paths. Missing: redirect ownership completion, broader keyctrl details, palette mutation, Bind/ParentVar, push/collision/HitDef/Target, and pause semantics. Evidence: focused entity/runtime/render/lifecycle, special-State ordering, debug-gating, Trigger-group cache, compiled parity, State-index, and Redirect tests plus local itoko production-loader verification. |
...
| MoveHit | Partial 85% | Implemented: Returns 1 on accepted hit, remains 1 through attacker hitpause, and increments on later unpaused ticks. Later HitDef activation preserves the result until a new contact or explicit/state reset, allowing bundled T-H-M-A State 1011 to route to 1012 after its first hit while retaining State 1016 multihit generations. Missing: Helper/projectile parity, MoveReversed, and broader WinMUGEN lifetime cases. Evidence: focused MoveContact timing, live HitDef, and bundled T-H-M-A State 1011/1016 tests. |
