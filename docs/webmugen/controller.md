...
`TargetState` assigns the controller owner's stable root player id as the target's State-document owner and also records the unique root or Helper controller entity. This matches HitDef `p2stateno` with `p2getp1state = 1`; redirects in the borrowed State use the supplying entity's Root/Parent relationships, while `SelfState` returns the target to its own document and clears that entity ownership. Issue #120 verifies Helper 1462's real State 1465 `root,movetype = H` abort route. Root targets selected by a Helper are covered; Helper-as-target, animation ownership, teams, and multi-player targets remain Partial. `TargetBind` immediately applies the Facing-relative position and owner velocity, then repeats that synchronization after entity physics for every effective bind tick, including the tick that reduces a finite duration to zero. A contact-conditioned TargetBind with `ignorehitpause = 1` receives a narrow post-contact pass after Target registration and before `p2stateno` Time=0 execution. This reproduces itoko Helper 1462's authored order: grounded P2 is moved to `pos = 85,10` before State 1465 tests `Pos Y >= 0`, rather than immediately returning to State 5100. Stage clamping and push resolve the unique owner from roots or Helpers, apply the same final correction, and only then clear expired metadata. Issue #134 covers both boundaries. Omitted `time` lasts one tick, zero cancels a bind, and any negative value is normalized to indefinite; `TargetDrop` also clears a bind owned by the dropped target owner. A finite duration freezes while the controller owner is in P1 pause, but target-only P2 hit-shake still consumes it. Issue #125 verifies both itoko's two-tick opening release, Projectile 3066 launch arc, and the final one-tick ground bind that clears the scripted downward velocity before P2 returns through its own State 5120. Exact post-contact ordering for non-TargetBind controllers, global Pause/movetime combinations, Helper-as-target/team/multi-player ownership, and mixed coordinate-space scaling remain Partial.
...
`Trans` is a tick-scoped Root/Helper sprite override. The runtime evaluates both `trans` and the optional two-component `alpha` expression in the executing entity's current context, and Canvas rendering applies the resulting `none`, `add`, `add1`, `addalpha`, or `sub` mode to that entity's current AIR sprite. `trans = default` (or no active Trans controller) returns selection to the AIR element's blend field. A Controller override takes precedence over AIR for that tick. Bundled akkarin State 1200's fade therefore resolves `alpha = 8*time,256-8*time` to `80,176` at Time 10 and uses `150,100` from Time 19 onward for its Helper. Its State -3 normal-state `addalpha 256,0` is treated as opaque source replacement rather than Canvas additive brightening. During HitPause, a Trans controller suppressed by the ordinary `ignorehitpause = 0` rule leaves the last presented blend intact; an `ignorehitpause = 1` Trans may overwrite it. On the next active tick, normal tick-scoped reset and trigger evaluation resume. Other additive composition uses the source alpha directly; exact WinMUGEN destination-alpha framebuffer arithmetic and broader global Pause timing remain Partial.
...
`Pause` and `SuperPause` emit match-level events. `time` starts on the Controller execution frame. The owner may continue State processing, animation, Controller evaluation, collision eligibility, and physics for `movetime`; other root CNS and physics stop, including negative States. Equal `time`/`movetime` keeps the owner active throughout the Pause. On the first post-Pause pass, only entities that were actually frozen receive the one-tick resume guard; an owner whose `movetime` covered the final Pause tick evaluates its next authored `Time` normally. Explods use their own matching allowance. PlaySnd on a frozen activation pass fires once, then controller suppression plus the resume guard prevents replay. Existing browser voices continue through the pause. A SuperPause with `darken = 1` draws a half-opacity black screen pass after regular layers and before hit feedback/`ontop` Explods; normal Pause and `darken = 0` do not. Same activation-pass cross-player ordering, Helper-specific resume ownership, default SuperPause anim/sound/pos, and other presentation details remain Partial.
...
The matching ground/air/down/guard/airguard cornerpush value creates a separate attacker position offset only when the contacted root target touches the current camera containment edge; the legacy fallback-stage check remains when no camera edge marker is available. The value is converted by attacker Facing once, freezes during HitPause, then decays by the WinMUGEN-specific `0.7` coefficient until its next magnitude is below `1`. It is not combined with attacker X velocity or character ground friction. Omitted cornerpush parameters use the WinMUGEN default/inheritance chain, including non-air `ground = 1.3 * guard.velocity` and air-attribute `ground = 0`. `snap` places the target at attacker position plus Facing-relative X and absolute Y offsets. `p1sprpriority` and `p2sprpriority` update the two runtime sprite priority fields on hit or guard, using the WinMUGEN omitted defaults `1` and `0`; Canvas interleaves normal Player/Helper/Explod sprites from lower priority to higher priority. Helper/team cornerpush and Projectile sprite-priority integration remain Partial.
...
During the selected hit time, the runtime keeps `ctrl = false`, blocks control-enabling `CtrlSet`, and blocks early recovery from the current positive State to State 0/52. State -1 itself remains data-driven outside HitPause: every controller whose triggers pass may execute, including authored guard cancels that explicitly select guard States 150-153. This follows Elecbyte's WinMUGEN 3 Sep 1999 guard-cancel example; the engine does not infer guard cancel from `GetHitVar(guarded)` or a destination State number. Ordinary movement/attack routes remain unavailable when their own `Ctrl`, StateNo, Power, or other conditions fail. During HitPause, ordinary Controllers still require `ignorehitpause = 1`; the narrow WinMUGEN compatibility exception is an automatic, non-Command State -2 `ChangeState`. Issue #131 requires this path for bundled itoko's `var(25) >= 100` guard-break route to enter State 2031 before the following paused reset clears the value. Internal common get-hit transitions such as State 5000 to 5001 remain available. Hit-stun elapsed time is stored independently from `stateTime` so internal get-hit State changes do not shorten the configured duration. When that time expires in a borrowed `TargetState`, fallback recovery retires the hit-stun timer without forcing State 0; the attacker's CustomState remains responsible for `ChangeState` and the eventual `SelfState` return.
