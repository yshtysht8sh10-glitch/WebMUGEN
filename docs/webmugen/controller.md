...
Issues #30-#32 connect `Explod` creation, owner AIR/SFF rendering, and baseline lifecycle. Issue #33 connects `ModifyExplod` for explicit ids: every matching Explod in the exact owner scope receives a partial update, duplicate ids are all updated, missing ids are safe no-ops, changed coordinates and draw fields reach Canvas in the same frame, and changed removetime starts a fresh clock. Internal runtime ids remain independent from duplicate MUGEN ids. Omitted ModifyExplod id semantics, Remove/Bind controllers, movement integration, Pause/SuperPause gating, Helper/fightfx ownership, and exact camera/random semantics remain Partial per `explod-integration-design.md`.
