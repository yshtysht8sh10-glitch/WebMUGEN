...
Issues #30-#32 connect `Explod` creation, owner AIR/SFF rendering, and baseline lifecycle. Issue #33 connects explicit-ID `ModifyExplod`; Issue #38 connects explicit-ID `RemoveExplod`. Every matching Explod in the exact owner scope is updated or removed, duplicate ids are all selected, and missing ids are safe no-ops. Create/modify/remove events retain CNS controller order, and removal excludes entries from lifecycle and Canvas in the same frame. Internal runtime ids remain independent from duplicate MUGEN ids. Omitted Modify/Remove id semantics, ExplodBindTime, movement integration, Pause/SuperPause gating, Helper/fightfx ownership, and exact camera/random semantics remain Partial per `explod-integration-design.md`.
