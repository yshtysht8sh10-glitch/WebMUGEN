...
Issue #81 connects `AfterImage` to player frame-history capture and Canvas rendering. `time`, `length`, `timegap`, `framegap`, and `trans` follow the documented history semantics; `AfterImageTime` changes an active effect and zero clears it. Each captured frame retains AngleDraw angle/scale, and Canvas reapplies that historical transform around the AIR-offset-adjusted sprite axis without rotating the AIR displacement. The palette fields are retained and approximated with Canvas filters, while `add`/`add1` use additive composition. Exact indexed-palette arithmetic, subtractive blending, and pause behavior outside the root-player path remain Partial. `raw.afterimage_draw` reports captured/displayed/drawn counts and the active gaps.
