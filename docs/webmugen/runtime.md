...
Issue #93 connects the presentation states around that combat result. Both roots enter engine Initialize State 5900 at RoundState 0, preserving the character/common `5900 -> 190 -> 191` first-round route and its variable initialization. Their character-owned 190-199 routes run at RoundState 1 until both have finished or stopped asserting `intro`; any mapped game input can end those character Intro states, after which the HUD still presents `ROUND N` and `FIGHT!`. A later round with no selected intro enters controllable State 0 when Fight begins, including StateDefs that omit `ctrl`. Fight input/collision begins at RoundState 2. KO waits for an airborne winner to land before dispatching State 180, while the loser remains on the common down route to State 5150. The result presentation timer begins only after the win State is entered, so landing time does not shorten the victory animation. State 170 and draw State 175 are reserved for time-over presentation. `roundnotover` delays restart while a victory State asserts it. After two wins, the current single-match application clears the score and automatically starts a new match at Round 1.

The game viewport owns a persistent Japanese/English toggle. UI chrome, help, settings, and status labels select one language at a time; character/CNS identifiers and raw diagnostic payloads remain their source-format technical data.
