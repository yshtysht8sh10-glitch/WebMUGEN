...
Issue #30 resolves initial position; #31 renders owner AIR/SFF; #32 steps animation/removetime/binds; #33 modifies; #38 removes; #39 changes bind duration. Create/modify/remove/bind-time callbacks share one ordered frame-local stream. ExplodBindTime updates every exact owner/id duplicate: 0 releases, a positive value uses finite follow/release, and a negative value follows indefinitely. An unbound P1/P2-postype entry rebinds from its stored offset, while postypes without a bind target remain unbound. Missing parameters/ids are diagnosed safe no-ops. Round restart constructs a new empty collection. Omitted-ID selection, non-player owner lifecycle, movement integration, and Pause/SuperPause gating remain unresolved scopes.
