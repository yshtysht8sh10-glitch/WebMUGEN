...
Issue #30 resolves the initial legacy postype/Facing transform exactly once and stores both the original offset and resolved stage/screen position. Issue #31 selects owner AIR/SFF and submits regular or `ontop` effect drawables. Issue #32 steps existing entries after CNS execution but before physics/render, while creation-frame entries stay at age/time zero. It advances AIR element/time, applies removetime, updates/release binds, and filters removed entries before Canvas consumption. Round restart constructs a new empty collection. Movement and Pause/SuperPause gating remain later scopes.
