...
Issue #81 includes existing Helpers as attack candidates after the root-player priority pass. Each active Helper HitDef uses that Helper's owner AIR, position, facing, Anim and Clsn1 against eligible root Clsn2 boxes. HitDef `affectteam` defaults to enemy and permits enemy, friendly, or both roots for E/F/B. Accepted contact mutates the root defender through the normal damage/guard/reaction/effect path and writes HitPause, MoveContact/MoveHit, hit-target consumption and Target registration back to the unique Helper entity. `raw.helper_hit_collision` keeps the runtime Helper id visible even though character-owned hit effects use the root id. Helper-as-defender, Helper-vs-Helper, team partners and simultaneous priority interactions remain Partial.
