...
Issue #30 resolves the initial legacy postype/Facing transform exactly once and stores both the original offset and resolved stage/screen position. Issue #31 selects owner AIR/SFF and submits regular or `ontop` effect drawables. Issue #32 steps existing entries after CNS execution but before physics/render, while creation-frame entries stay at age/time zero. It advances AIR element/time, applies removetime, updates/releases binds, and filters removed entries before Canvas consumption. Issue #33 applies explicit-ID `ModifyExplod` events after CNS evaluation and before lifecycle/render. All exact owner/id duplicates receive the partial patch; omitted fields remain unchanged, coordinate changes re-resolve immediately, animation restarts only when changed, and removetime starts a fresh clock on the modification frame. Missing or omitted ids are diagnosed safe no-ops. Round restart constructs a new empty collection. Omitted-ID selection, movement integration, and Pause/SuperPause gating remain later or unresolved scopes.
