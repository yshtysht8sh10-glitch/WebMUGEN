...
`GameState.pause` is the match-level Pause/SuperPause clock. The activation CNS pass emits one owner-scoped event; subsequent paused passes skip negative and current State controllers for non-moving entities. The controller owner continues State processing, animation, Controller evaluation, collision eligibility, and physics for its `movetime`; other players, round time, hit resolution, and physics remain frozen. `time = movetime = 12` therefore counts 12 activation-inclusive frames for the owner and zero moving frames for the opponent. HitOverride destinations execute their Time=0 Controller pass during the collision entry, so T-H-M-A State 902 starts Pause and PosFreeze on contact rather than one frame later. A one-frame resume guard prevents activation-side-effect replay after Pause ends. Canvas rendering consumes the same state: an active SuperPause with `darken` enabled darkens regular layers while leaving hit feedback and `ontop` Explods above the darkening pass. Same-pass events emitted by another player before the coordinator applies the pause remain a documented Partial boundary.
