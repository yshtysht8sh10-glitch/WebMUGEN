...
Issue #30 resolves the initial legacy postype/Facing transform exactly once and stores both the original offset and resolved stage/screen position. Issue #31 selects AIR/SFF from the entry's owner, resolves the current element, converts stage coordinates through the Canvas camera origin, and submits regular or `ontop` effect drawables. The renderer does not mutate the entry and missing assets produce hidden diagnostics. Round restart constructs a new empty collection. Animation/removetime/bind stepping remains assigned to #32.
