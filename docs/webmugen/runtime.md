...
Issue #93 connects the presentation states around that combat result. Both roots enter PreIntro State 190 at RoundState 0; their character-owned 190-199 routes run at RoundState 1 until both have finished or stopped asserting `intro`. Fight input/collision begins at RoundState 2. Round-over dispatches winner State 180, loser State 170, or draw State 175 and keeps their ordinary CNS/animation/effect paths active. A non-final result starts the next RoundNo after the result window; two wins expose MatchOver/RoundState 4. Continue/select-screen navigation is not part of the current single-match runtime.
