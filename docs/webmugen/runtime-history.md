...
For keyboard gestures, the same non-editable `BrowserInput` keydown that adds `KeyA` (or another game key) calls the shared audio gesture callback synchronously. Therefore `audio_context_created` and `audio_unlock_requested userGestureType=keydown` precede the State/PlaySnd frames produced by that key, even after an F5-created fresh runtime. Pointer unlock remains independent. No Runtime or Settings tab transition participates in either route.
