...
`raw.global_pause` records Pause/SuperPause activation kind, owner, time, movetime, darken, and the continuing-audio policy. CNS traces use `global_pause skip` with the pause kind or `resume_guard`; a fully moving owner has no `resume_guard` skip on the post-Pause pass, while frozen opponents retain it. Frozen Explods emit `raw.explod_step ... result=frozen` with the matching allowance; allowed ticks emit `raw.explod_pause` with allowance before/after values. A negative indefinite allowance keeps the same before/after value and adds `mode=indefinite`. When an active SuperPause has `darken = 1`, Canvas emits `raw.superpause_darken` with remaining time, opacity, and its `before_ontop` layer position.

`raw.helper event=spawn` records the evaluated Helper Size scale and separate Pause/SuperPause move-time allowances together with identity, ownership, State, Anim, `firstStep=creation_frame`, and `creationRender=immediate`. Every accepted Helper uses the same creation-frame contract regardless of parentage. A request rejected at WinMUGEN's total ceiling records `result=helpermax limit=56`. `raw.render` includes the effective Helper draw scale.

`raw.state_transition` records `result=limit count=64` only when an invalid same-tick ChangeState cycle reaches the runtime safety ceiling. Valid multi-State routes continue through every destination without emitting this record.
