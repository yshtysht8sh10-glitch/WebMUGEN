...
`raw.helper event=spawn` records the evaluated Helper Size scale and separate Pause/SuperPause move-time allowances together with identity, ownership, State, Anim, deferred first-step timing, and `creationRender=immediate|after_initial_pass`. `raw.render` includes the effective Helper draw scale.
