...
| Projectile/Helper/Explod | Explod creation is connected from normal CNS execution to an owner-scoped production `GameState` model with separate runtime/MUGEN ids and diagnostics. Animation/render/lifecycle and Explod mutation controllers remain incomplete; see `explod-integration-design.md`. Helper/Projectile also remain incomplete. |
