...
| Target / bind integration | Root and uniquely identified Helper Target owners survive CNS physics and the final stage clamp/push correction. Contact first registers the Target, then runs matching `ignorehitpause = 1` TargetBind before `p2stateno` Time=0. Focused Issue #134 coverage starts with real itoko Helper 1462 hitting grounded P2, verifies State 1465 and `pos = 85,10`, and follows the bind through release and Helper 1472 re-catch. |
| State entry | Centralized entry applies several StateDef header fields. |
| Physics | Basic position/velocity stepping exists; S/C uses character `stand.friction`/`crouch.friction` with WinMUGEN defaults. Full WinMUGEN physics remains Partial. |
| Animation | AIR-based animation selection and timing exist; exact edge cases need audit. |
| Hit/Collision | Partial live paths cover normal/guard/fall kill separation, explicit power/numhits, camera-containment-aware root cornerpush with a dedicated WinMUGEN `0.7`-decayed offset, snap, sprite priority, effects, chains, targets, and common get-hit routing. Three-character P1/P2 real-data regression evidence is recorded in `hitdef-real-character-regression.md`; projectile/team/Helper-defender behavior remains a major area. |
