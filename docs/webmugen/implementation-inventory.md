...
| Build mode | One explicit Development/Public baseline keeps server-writer features such as Character/Stage editing and default export Development-only. Public starts locked and can elevate the current tab through the server-validated Development Mode Pass; reload fails closed to Public. Catalog management/generation and Runtime diagnostics remain available in both modes. |
