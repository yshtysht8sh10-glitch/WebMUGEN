...
| Projectile/Helper/Explod | Explod create/render/lifecycle, explicit-ID mutations, motion/render extensions, owner-hit removal, round cleanup, and Pause/SuperPause per-entry allowance gating are production-connected. Palette isolation, destination/subtractive blend, shadows, and non-player owners remain incomplete; see `explod-integration-design.md`. Helper/Projectile also remain incomplete. |
