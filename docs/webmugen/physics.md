...
Horizontal separation is applied only when both Size rectangles overlap vertically and horizontally. `PlayerPush = 0` disables separation for the frame in which the controller executes; it returns to enabled on the next CNS frame unless another `PlayerPush = 0` executes. Stage AutoTurn is evaluated for grounded root players after CNS motion and again after push/TargetBind position resolution, independent of character identity, State number, and `MoveType`. Airborne players preserve Facing through a cross-over; the opponent direction is applied when the player lands and becomes grounded. This corrects grounded cross-overs and landing without a State 3405-style route while avoiding mid-air flips. `[StageInfo] autoturn = 0` disables this stage rule. `AssertSpecial noautoturn` suppresses it for the asserting player and tick; custom attacks or TargetBind carries that must preserve direction are expected to assert that WinMUGEN flag. A player frozen by hitpause or global pause is not turned until its runtime tick can move again. Explicit StateDef `facep2` still applies only on State entry, and the `Turn` Controller still performs an explicit reversal.

This remains Partial compatibility because dynamic `Width` controller overrides and broader multi-entity push behavior still need integration. After push and fallback stage clamping, the selected 320x240 or extended 400x240 viewport computes one shared camera. A root whose `ScreenBound value` is enabled is shifted horizontally until its Size-derived Push Box remains inside the camera using the external Stage DEF `[Bound] screenleft`/`screenright` insets, or the four-logical-pixel fallback for built-in stages; velocity is preserved. `value = 0` bypasses the correction. `raw.push` records both resolved boxes, their Size source, overlap and skip/apply result; `raw.cross` records airborne status, each player's `noAutoTurn` value, stage AutoTurn enablement, and Facing changes; `raw.camera` records the final origin and corrected players. Canvas debug draws the same resolved push rectangle in blue.
