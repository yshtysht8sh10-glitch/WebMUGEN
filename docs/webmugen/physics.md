...
Issue #62 keeps KO and fall recovery as separate common-state routes. State 5150 remains lying and uncontrollable. State 5200 falls under its CNS `GetHitVar(yaccel)` until it enters 5201; State 5201 applies its data-defined ground recovery launch. State 5210 executes boolean `PosFreeze` at Time 0, so physics skips movement for that one frame without pausing StateTime, then CNS steering/gravity continues to the normal State 52 landing route.
