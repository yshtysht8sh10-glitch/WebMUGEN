...
Horizontal separation is applied only when both Size rectangles overlap vertically and horizontally. `PlayerPush = 0` disables separation for the frame in which the controller executes; it returns to enabled on the next CNS frame unless another `PlayerPush = 0` executes. Stage AutoTurn is evaluated for both root players after CNS motion and before push resolution, independent of character identity, State number, `StateType`, and `MoveType`. It therefore corrects Facing after grounded or airborne cross-overs and on landing without a State 3405-style route. `[StageInfo] autoturn = 0` disables this stage rule. `AssertSpecial noautoturn` suppresses it for the asserting player and tick; custom attacks or TargetBind carries that must preserve direction are expected to assert that WinMUGEN flag. A player frozen by hitpause or global pause is not turned until its runtime tick can move again. Explicit StateDef `facep2` still applies only on State entry, and the `Turn` Controller still performs an explicit reversal.
