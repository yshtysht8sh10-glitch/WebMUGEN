...
StateDef `type` and `physics` establish their values on entry only. A Time=0 `StateTypeSet` can
therefore keep an `A`/`N` route active across later ticks; State 3410 uses this to select its airborne
front-edge exit after a missed Darkness Finger rush.
The same rule applies after CNS has already placed an `S/C` entity below the ground axis: it is not
teleported back to the floor. T-H-M-A State 3730 therefore retains Pos Y=400 beneath its State 3735
rock Helper at Pos Y=360 while both begin their authored lift.
...
- `PosSet` converts its screen-center-relative X through the current frame camera center into internal world X after evaluating CNS expressions, while `PosAdd` adds a Facing-relative X offset and an axis-space Y offset;
...
Creation-frame Helpers follow the same controller-before-physics order. Kikka's Helper State 2000
first applies `VelAdd y = .15`, then its later `root, AnimElem = 1` `VelSet` overwrites the launch to
`(.15,-3.2)` on the creation frame. `Physics=N` integrates that velocity without adding gravity or
clamping to ground; later ticks execute only the always-true `VelAdd`, producing `vy=-3.05,-2.90,...`.
...
while its later ground bind clears the scripted downward velocity before release. Global
Pause/movetime combinations and Helper-as-target ownership timing remain Partial.
...
Projectile contact uses the same split between world-space live velocity and the WinMUGEN-relative HitDef snapshot. Before contact is accepted, it now uses the same target-class eligibility as an ordinary HitDef: a lying `StateType = L` target requires `D` in addition to a normal H/L/A/M hitflag class. Accepted lying Projectile hits enter State 5080 and use `down.velocity`, `down.hittime`, and `down.bounce`; lying targets cannot guard the contact. It records `hitVel`, hit time, fall values, and `GetHitVar(yaccel)` before entering the common get-hit States with `Physics=N`. This is required because States 5030/5040 apply gravity explicitly; retaining only the initial projectile Y velocity would send a launched defender upward indefinitely.

Projectile `guardflag` uses the defender's current stand/crouch/air guard intent at collision. Accepted guard enters State 150/152/154 and applies `guard.velocity`; normal hit additionally starts any snapshotted defender PalFX. Player PalFX duration advances on game ticks, including HitPause, while exact Pause/SuperPause parity remains under audit.
...
Issue #60 audits the complete 5030 fall family against the unmodified common States. State 5030 restores contact velocity, 5035 handles the optional transition animation, 5040 handles non-fall recovery, and 5050 owns falling, recovery input, and contact with the ground. The number 5060 is an animation family selected by State 5050, not a `StateDef`. Trip State 5070 keeps motion and AnimTime frozen through defender hit-shake while StateTime follows game-time, and enters 5071 only when `HitShakeOver`; 5071 then restores hit velocity and applies `GetHitVar(yaccel)`.

`raw.gethitvar_frame` includes position/velocity, yaccel, ground crossing, fall/recover window, recovery input, and the ground-clamp decision for these States. `raw.gethit_changestate_eval` reports each ChangeState controller index and result. `raw.fall_pause` records hitpause or global pause frames; defender P2 hit-shake advances StateTime while attacker P1 pause freezes it, and both freeze animation/motion. Match-level Pause retains its separate clock rules.

Issue #61 separates lying-hit time from get-up time. A HitDef or Projectile permitted to contact `StateType = L` enters State 5080 and uses `down.velocity` plus `down.hittime`; nonzero Y launches through the 5090 animation and 5030, while zero Y slides through 5081. `down.bounce` decides whether a launched lying target receives one fall bounce. Omitted `down.velocity` inherits `air.velocity`.

Issue #62 keeps KO and fall recovery as separate common-state routes. State 5150 remains lying and uncontrollable. State 5200 falls under its CNS `GetHitVar(yaccel)` until it enters 5201; State 5201 applies its data-defined ground recovery launch. State 5210 executes boolean `PosFreeze` at Time 0, so physics skips position integration for that one frame without pausing StateTime/AnimTime or zeroing velocity, then CNS steering/gravity continues to the normal State 52 landing route. Repeated PosFreeze triggers, such as State 902 `Time <= 12`, apply the same one-frame rule on every matching owner-active frame.

State 5110 get-up scheduling uses an independent counter loaded from the defender's `[Data] liedown.time`. It freezes during hitpause/Pause, does not depend on StateTime or `down.hittime`, and never schedules 5120 at zero Life. Reaching the duration marks the player ready but leaves State 5110 intact through that render boundary. The following CNS pass performs the engine-owned 5110→5120 entry before State scans, allowing State 5120 Time=0 Controllers and an immediate character-defined route such as itoko State 730 to finish before the next draw. This avoids rendering one stale lying-animation frame under State 5120. `raw.down_clock` exposes elapsed/duration/remaining and the advance/frozen/ready/ko_hold result.

HitDef `pausetime = p1, p2` initializes separate attacker-pause and defender-shake counters. WinMUGEN defaults an omitted pair to `0,0`; `guard.pausetime` inherits that normal pair when omitted. While either counter is positive, ordinary CNS controllers, position, velocity integration, AnimTime, and hit-stun clocks are frozen; `ignorehitpause = 1` controllers still execute. P1 pause also freezes StateTime, whereas P2 hit-shake advances it once per game frame after the State entry/Time=0 pass. Standard guard contact therefore still lets itoko State 150 Time=1 add exactly 3 to root `var(25)` for Issue #131, while the Projectile 3066 P1 pause keeps zipper State 3006 far enough from `Time = 25` for the target's launch arc in Issue #125. HitPause is not collision immunity: an unpaused attacker with a different ActiveHitDef generation can still contact the frozen defender, while the generation/defender history rejects continued overlap from the already-consumed generation. A newly accepted contact replaces the defender's older counter with its own `p2` value; a same-frame trade keeps the longer of the two newly applied role counters and its role. A counter of zero resumes normally without an extra frozen frame. Input buffering remains active outside this per-player freeze. Match-level Pause/SuperPause is separate: it freezes non-moving players and round/hit stepping, permits the controller owner for `movetime`, and applies its resume guard only to entities that were frozen on the final Pause tick.
...
This remains Partial compatibility because negative dynamic `Width` values and broader multi-entity push behavior still need integration. `Width edge = front, back` overrides the screen-edge bar for one tick, `player = front, back` separately overrides the push bar, and `value` assigns both. After push, the selected 320x240 or extended 400x240 viewport computes one shared camera. The fixed fallback hard bound is applied only when no Stage runtime loaded; a loaded native or external Stage owns final containment through its camera rules. The fallback camera itself uses the same 48/912 world limits as fallback physics, including the classic 320-wide view. For an external Stage without a dynamic Width, an enabled root is shifted until its player axis retains the DEF `[Bound] screenleft`/`screenright` distance from the viewport edge; a dynamic edge width is honored for that tick. Built-in stages use the Size-derived edge bar or its dynamic override with their four-logical-pixel containment fallback. Velocity is preserved and `ScreenBound value = 0` bypasses the correction. `raw.push` records both resolved boxes, their Size/Width source, overlap and skip/apply result; `raw.cross` records airborne status, each player's `noAutoTurn` value, stage AutoTurn enablement, and Facing changes; `raw.camera` records the final origin and corrected players. Canvas debug draws the resolved player push rectangle in blue. This alignment lets T-H-M-A State 3420 satisfy `FrontEdgeBodyDist <= 20` and enter State 3430 instead of retaining velocity while being reset to X=912 indefinitely.

On a native Stage with authored horizontal `tension`, the camera follows the leading enabled root
until its Stage camera bound; ScreenBound containment shifts another enabled root only if it would
leave that moving viewport. This lets edge-triggered wall carries reach the authored Stage end. On
the legacy tension-less path, when sustained movement makes simultaneous root containment impossible, the camera is held at the
preceding legal point between the incompatible containment limits. The fixed viewport then clamps
only the root walking beyond its edge; the stationary opponent's world X is not rewritten. The
correction keeps velocity intact so the edge-bound root remains in its data-driven walk, dash, jump,
or reaction state rather than receiving a motion-specific engine rule.

Built-in stages retain the quarter-strength jump camera. External stages instead interpret `[Camera] startx`/`starty`, horizontal/vertical bounds, `tension`, `verticalfollow`, and `floortension`, then convert that WinMUGEN camera coordinate into the renderer's viewport origin. The mapping keeps Stage coordinate `(0, 0)` centered on the 320-wide base view and keeps `zoffset` on the runtime ground line, so changing between classic and extended view does not introduce an artificial camera offset. Stage BG placement separately treats `start` as top-center-relative and applies camera movement through `delta`; it does not add the gameplay `zoffset` a second time. Exact WinMUGEN camera hysteresis, PlayerInfo spawn/bound enforcement, and impossible spans taller than the viewport remain Partial.
...
## Current-viewport containment

Issues #120/#126 use player-axis ScreenBound containment unless a Width controller supplies an explicit edge. When two enabled roots cannot fit, the camera retains its preceding valid position and the escaping root is clamped at that viewport instead of dragging the stationary root toward the Stage boundary. `FrontEdgeDist` therefore reaches the authored near-zero threshold at the visible edge; `FrontEdgeBodyDist` continues to include Size/Width body geometry.
