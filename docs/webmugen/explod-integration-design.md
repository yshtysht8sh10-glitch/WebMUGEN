...
This document records the Issue #25 audit and the integration contract for the Explod roadmap. Issues #30-#33 connect creation, rendering, baseline lifecycle, and explicit-ID modification; Issue #38 connects explicit-ID removal.
...
Issue #31 resolves each visible entry through its owner-scoped AIR/SFF assets, applies world/screen conversion and Facing/vfacing once, and submits regular or `ontop` draw layers. Issue #32 advances age/AnimElem/time, implements exact creation-tick counting for removetime 0/positive/-1/-2, follows bind owners for the configured tick count, releases to the last world position, and removes entries before rendering. Issue #33 evaluates `ModifyExplod` through the normal CNS context and partially updates every explicit owner/id match. Issue #38 evaluates `RemoveExplod` in the same ordered controller event stream and removes every explicit owner/id match before lifecycle and Canvas rendering, including bound, looping, or persistent entries. Missing ids are diagnosed safe no-ops. Missing AIR actions or sprites remain hidden with diagnostics. Velocity/acceleration stepping remains #34; Pause/SuperPause gating remains #35. Omitted Modify/Remove id semantics, `random`, non-zero camera runtime, Helper/orphan ownership, fightfx asset loading, generic controller `persistent`, and `NumExplod` remain Partial.

## Audited implementation inventory

| Existing code | What works | Production status | Decision |
|---|---|---|---|
| `core/explod/ExplodSystem.ts` | Owner-scoped production runtime entries, separate runtime/MUGEN ids, ordered creation/modification/removal application, lifecycle, and diagnostics. | Connected to `GameState`, normal CNS effect controllers, the app coordinator, lifecycle, and renderer by #30-#33/#38. | Use as the durable model for #39 onward. |
...
| CNS compatibility shims | `ExplodBindTime` is recognized; `NumExplod` returns zero. | These remaining operations are safe no-ops with no game-state effect. | Keep Matrix status honest until each production path is connected and tested. |
...
`runtimeId` and `mugenId` must never be aliases. Explicit-ID `ModifyExplod` and `RemoveExplod` select every entry in the exact owner scope with the matching MUGEN id; internal code still uses `runtimeId` for one exact entry. Duplicate MUGEN ids remain representable. Omitted-ID Modify/Remove selection is intentionally a diagnosed safe no-op until a reliable WinMUGEN boundary rule is established. ExplodBindTime and NumExplod selection are implemented in their later Issues.
...
CNS controller trigger passes
  ...
  -> owner-scoped Explod create/modify/remove request is appended to one ordered frame stream
...
6. **#39**: owner-scoped ExplodBindTime selection semantics.
