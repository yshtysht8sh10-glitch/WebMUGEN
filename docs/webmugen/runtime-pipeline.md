...
Round presentation uses the same CNS pipeline. RoundState 0 enters both roots into engine Initialize State 5900; on round one, character/common CNS continues through State 190 into the character-owned 191-199 Intro family. States 190-199 and `AssertSpecial intro` keep RoundState 1 active until both characters finish. The HUD then presents `ROUND N` followed by `FIGHT!` while commands and collision remain gated; RoundState 2 begins only after that presentation. Later-round initialization restores controllable State 0 even when its StateDef omits `ctrl`. KO dispatches State 180 only to the winner and lets the defeated player finish the common route to State 5150; time-over dispatches State 180/170, or 175 to both on a draw. Result presentation advances from RoundState 3 to 4 independently of MatchOver, and `AssertSpecial roundnotover` delays the non-final restart until the victory state releases it. See `../mugen-spec/winmugen/round-flow.md`.
