...
Round presentation uses the same CNS pipeline. RoundState 0 enters both roots into State 190, then States 190-199 and `AssertSpecial intro` keep RoundState 1 active until both characters finish. Commands and collision are gated until RoundState 2. KO/time-over dispatches State 180 to the winner and 170 to the loser; a draw dispatches 175 to both. Non-final results restart through State 190 with an incremented RoundNo, while two wins expose match-over RoundState 4. See `../mugen-spec/winmugen/round-flow.md`.
