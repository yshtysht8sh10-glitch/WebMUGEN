...
Round presentation uses the same CNS pipeline. RoundState 0 enters both roots into engine Initialize State 5900; on round one, character/common CNS continues through State 190 into the character-owned 191-199 Intro family. States 190-199 and `AssertSpecial intro` keep RoundState 1 active until both characters finish. The HUD then presents `ROUND N` followed by `FIGHT!` while commands and collision remain gated; RoundState 2 begins only after that presentation. KO/time-over dispatches State 180 to the winner and 170 to the loser; a draw dispatches 175 to both. Result presentation advances from RoundState 3 to 4 independently of the separate MatchOver trigger. Non-final results restart through State 5900 with an incremented RoundNo. See `../mugen-spec/winmugen/round-flow.md`.
