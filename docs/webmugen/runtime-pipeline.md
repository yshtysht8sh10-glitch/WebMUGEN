...
Match-level Pause/SuperPause is checked before State -3/-2/-1 and the current StateDef. Non-moving players therefore emit no repeated Explod or sound events while paused. The controller owner runs only while its `movetime` remains; physics, round time, hit resolution, and Explod lifecycle use the same frame's pause snapshot. Explods and Helpers consume their matching `pausemovetime` or `supermovetime` independently; a moving Helper advances its CNS, physics, and animation clocks while the allowance remains. When the clock reaches zero, one guarded CNS pass advances StateTime through physics without replaying time-zero controllers. Already-started Browser Audio continues; the runtime does not suspend AudioContext.
...
Round presentation uses the same CNS pipeline. RoundState 0 enters both roots into engine Initialize State 5900; on round one, character/common CNS continues through State 190 into the character-owned 191-199 Intro family. States 190-199 and `AssertSpecial intro` keep RoundState 1 active until both characters finish; mapped game input routes both roots to State 0 to skip only that character Intro portion. The HUD then presents `ROUND N` followed by `FIGHT!` while commands and collision remain gated; RoundState 2 begins only after that presentation. Later-round initialization restores controllable State 0 even when its StateDef omits `ctrl`. KO lets an airborne winner continue normal CNS/physics until grounded, then dispatches State 180 and starts the result timer; the defeated player finishes the common route to State 5150. Time-over dispatches State 180/170, or 175 to both on a draw. `AssertSpecial roundnotover` delays restart. A two-win MatchOver automatically resets the score and starts a new Round 1 match. See `../mugen-spec/winmugen/round-flow.md`.
