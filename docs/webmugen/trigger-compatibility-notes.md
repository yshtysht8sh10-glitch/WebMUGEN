...
| `AnimElem` | Complete | Uses 1-based AIR element starts, including redirected entities, comparison-time syntax, invalid-element false, and WinMUGEN finite-loop non-retrigger semantics. | Focused trigger/AIR tests plus bundled itoko State 1464's one-frame redirected launch and T-H-M-A State 101 PlaySnd / State 3010 Helper regressions cover the runtime path. |
...
| `HitShakeOver` | Partial 80% | Bare and numeric `HitShakeOver = 1` / `= 0` forms read the defender hit-pause counter. Jecht State 3405 now enters State 3406 when hit shake ends instead of remaining in the borrowed custom State. | Uncommon Pause/SuperPause and Helper interactions remain incomplete. |
...
Issue #118/#127 coverage verifies redirected command ownership: a non-keyctrl Helper cannot consume root commands through bare `Command`, but `root, Command` evaluates with the root command set. D4meirin coverage verifies the reciprocal keyctrl rule: a root evaluating `helper(99999), Command = "a"` sees its own resolved `a` command only when that Helper was created with `keyctrl = 1`. Issue #124 verifies that bare `Power` in a Helper observes the shared root gauge.
