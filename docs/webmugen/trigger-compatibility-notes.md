...
| `BackEdgeBodyDist` | Partial 90% | The compiled evaluator selects the Facing-relative back viewport edge and measures from the Size-derived ground/air push box; focused viewport/both-Facing tests and State 250 to 281 cover it. | Scrolling camera origin and dynamic Width overrides remain incomplete. |
| `FrontEdgeBodyDist` | Partial 90% | The compiled evaluator selects the Facing-relative front viewport edge and measures from the Size-derived ground/air push box; focused viewport/both-Facing tests and State 250 to 281 cover it. | Scrolling camera origin and dynamic Width overrides remain incomplete. |
...
| `P3Name` / `P4Name` | Partial | The current versus-only runtime returns the specified absent-player equality/inequality behavior. | Teammate and secondary-opponent slots are not implemented. |
| `PalNo` | Partial | Reports the first resolved DEF palette slot selected by the loader, defaulting to 1 without an external palette. | Character-select palette choice is not implemented. |
| `MoveContact` / `MoveHit` / `MoveGuarded` / `HitCount` | Partial | Reads move-level contact/hit/guard state and State-local hit count. Root, Helper, and Projectile contacts are connected; bundled T-H-M-A State 1005 and 1011 routes are tested. | MoveReversed and broader WinMUGEN lifetime cases remain incomplete. |
