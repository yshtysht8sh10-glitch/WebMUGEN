...
| `MoveContact` / `MoveHit` / `MoveGuarded` / `HitCount` | Partial | Reads ActiveHitDef-generation contact/hit/guard flags and State-local hit count; live normal/guarded routes and the Issue #65 target-confirm route are tested. | Helper/projectile parity and broader WinMUGEN lifetime cases remain incomplete. |
